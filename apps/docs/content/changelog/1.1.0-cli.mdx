...
Na função <CodeSpan.Method identifier="setupCreators"/>, defina uma função que será executada quando ocorrer um erro em algum evento:
...
A função <CodeSpan.Method identifier="block" /> funciona igual nos middlewares de comandos e eventos, mas com uma pequena adição:
...
Foi adicionado um ajudante para funções que respondem opções <CodeSpan.Method identifier="autocomplete"/>, agora ao invés de usar o método <CodeSpan.Method identifier="respond" /> da interação, e limitar o array de escolhas, você pode apenas retornar o array que a base fará isso para você, veja:
...
<Callout type="info">
	Você ainda pode usar o método <CodeSpan.Method identifier="respond" /> da interação se preferir:
	```ts
		return choices; // [!code --]
		interaction.respond(choices.slice(0, 25)) // [!code ++]
	```
...
A posição dos argumentos da função <CodeSpan.Method identifier="middleware" /> dos `responders` foi alterada para condizer com as outras funções de <CodeSpan.Method identifier="middleware" />:
...
- Foi adicionada na documentação, informações sobre o função <CodeSpan.Method identifier="autocomplete"/>, opção global dos comandos e de todas as novidades deste blog.
