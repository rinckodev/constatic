...
With this we can create a <CodeSpan.Class identifier="Responder" /> that expects any button component that follows this pattern in the customId.
...
<Callout type="danger">
    You can use this feature with any type of <CodeSpan.Class identifier="Responder" />, but don't forget that discord has a 100 character limit on customIds.
</Callout>

## Transforming CustomID parameters

The <CodeSpan.Method identifier="createResponder"/> function has an option where you can specify a way to transform the parameters object, which initially has both keys and values that are just strings. Check it out:
