...
To reply to a modal with <CodeSpan.Class identifier="Responder" />, add <CodeSpan.Enum identifier="ResponderType" member="Modal" /> to the types:
...
You can use the <CodeSpan.Enum identifier="ResponderType" member="ModalComponent" /> type:
```ts title="responder.ts"
createResponder({
    customId: "/register/form",
