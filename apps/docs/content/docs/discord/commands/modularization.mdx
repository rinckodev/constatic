...
The <CodeSpan.Method identifier="createCommand"/> function returns a `CommandInstance` that contains two special methods: <CodeSpan.Method identifier="group" /> and <CodeSpan.Method identifier="subcommand" />. As you can imagine, this is used to create groups and subcommands:
...
Since we exported the return of the <CodeSpan.Method identifier="createCommand" /> function using `export default`, we can import it into the `group.ts` files in the **channels** and **roles** folders to define the subcommand groups:
...
Above, we have something like a database query to get the document from our store system. When we return something within <CodeSpan.Method identifier="run"/>, it will be passed as the second argument to <CodeSpan.Method identifier="run"/> of the groups and subcommands, all with automatically inferred typing!
