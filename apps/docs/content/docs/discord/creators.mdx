...
On this base, to create commands and systems you must use the creator functions, which are <CodeSpan.Method identifier="createCommand"/>, <CodeSpan.Method identifier="createEvent"/> and <CodeSpan.Method identifier="createResponder"/>.
...
You can set some command options in the <CodeSpan.Method identifier="setupCreators"/> function:
...
<Tabs items={["Default permissions", "Guilds", "middleware", "onError", "onNotFound"]}>
  <Tab id="commands-default-permissions" value="Default permissions">
    Set what the default permissions will be for all commands, if none are defined with the <CodeSpan.Prop identifier="defaultMemberPermissions" /> option (It is the same permissions array as the command).
    ...
    ```
    This way, when you create a command and do not set any default member permissions, what you set in the <CodeSpan.Method identifier="setupCreators"/> function will be the default for all commands.
  ...
  <Tab id="commands-middleware" value="middleware">
    You can set a <CodeSpan.Method identifier="middleware" /> function that will be executed before the commands` run function
    ...
    You can call the <CodeSpan.Method identifier="block" /> function to block the execution of commands.
  ...
  <Tab id="commands-verbose" value="verbose">
...
<Tabs items={["middleware", "onError", "onNotFound"]}>
  <Tab id="responders-middleware" value="middleware">
    You can set a <CodeSpan.Method identifier="middleware" /> function that will be executed before the responders` run function
    ...
    You can run the <CodeSpan.Method identifier="block" /> function to block responders from executing.
  ...
  <Tab id="responders-onnotfound" value="onNotFound">
    ...
    ```
    <Callout type="danger">
        Be careful! If you use component collectors or modals in any command, this function will still be called, as it looks for a handler created with the <CodeSpan.Method identifier="createResponder"/> function. So it is important to check if the interaction has already been responded to or wait a short time before actually responding.
    </Callout>
  </Tab>
</Tabs>
...
You can set some event options in the <CodeSpan.Method identifier="setupCreators"/> function:

<Tabs items={["middleware", "onError"]}>
  <Tab id="events-middleware" value="middleware">
    You can set a <CodeSpan.Method identifier="middleware" /> function that will be executed before the events <CodeSpan.Method identifier="run"/> function
    ...
    <Callout type="info">
        In the first argument, an object with the event data is received, where <CodeSpan.Prop identifier="name" /> is the name of the event emitted and <CodeSpan.Prop identifier="args" /> are the arguments of that event, so checking what the name of the event is, the typing of the arguments is automatically defined:
        ```ts
        /* ... */ setupCreators({
            events: {
                async middleware(event) {
    ...
    You can run the <CodeSpan.Method identifier="block" /> function to block the execution of events.
    ...
    In the case of events, it is also possible to pass tags as arguments to the block function. This way, you can have 3 events of the same type, for example <CodeSpan.String magic content="messageCreate" />, but block only those with the previously defined tags:
  ...
  <Tab id="events-middleware" value="onError">
