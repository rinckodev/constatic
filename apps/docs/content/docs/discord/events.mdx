...
To create a function for a discord event listener, use the <CodeSpan.Method identifier="createEvent"/> function
...
Use the <CodeSpan.Prop identifier="name" /> property to create a custom identifier for your event, then set which discord event you expect using the `event` property.
```ts
import { createEvent } from "#base";
