...
import { ApplicationCommandData, ApplicationCommandType, AutocompleteInteraction, CacheType, ChatInputCommandInteraction, Client, Collection, MessageContextMenuCommandInteraction, UserContextMenuCommandInteraction } from "discord.js";

type CommandStoreData = Map<any, any> | Set<any> | Collection<any, any> | Array<any>; 
...
interface ChatInputCommandProps<D, Store> {
    ...
    autoComplete?(interaction: AutocompleteInteraction<GetCache<D>>, store: Store): void;
    run(interaction: ChatInputCommandInteraction<GetCache<D>>, store: Store): void;
    type: ApplicationCommandType.ChatInput
}
interface UserContextCommandProps<D, Store> {
    type: ApplicationCommandType.User
    run(interaction: UserContextMenuCommandInteraction<GetCache<D>>, store: Store): void;
}
interface MessageContextCommandProps<D, Store> {
    type: ApplicationCommandType.Message
    run(interaction: MessageContextMenuCommandInteraction<GetCache<D>>, store: Store): void;
}
...
type CommandData<D extends boolean, Store extends CommandStore> = 
...
export class Command<D extends boolean, Store extends CommandStore = CommandStore> {
    public static commands: Collection<string, CommandData<boolean, any>> = new Collection();
